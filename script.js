const correctPassword = "eskopi"; // Set your developer password here

function checkDate() {
    const today = new Date();
    const currentDay = today.getDate();
    const specialDate = new Date(today.getFullYear(), 7, 5); // 7 is August (0-based index)
    ...
    if (daysDiff > 0) {
        document.querySelector('.days-left').textContent = `There are ${daysDiff} days left until 5th August.`;
        document.getElementById('passwordSection').classList.add('active');
    } else if (daysDiff === 0) {
        startCountdown();
    } else {
        document.querySelector('.days-left').textContent = `5th August has passed.`;
        document.getElementById('passwordSection').classList.add('active');
    }
}
